$(function () {
	...
	$('.banner ol li').click(function (event) {
		...
		if (index > num) {
			...
			$('.banner ul li').eq(num).stop().animate({
				left: -win_w
			}, 500);
		} else if (index < num) {
			$('.banner ul li').eq(index).css({
	...
	// 提交订单的复选框
	$('.address').on('click', '.checkbox', function (event) {
	...
	$('.brand li').on('click', function (event) {
	...
	$(".nav_list").find('li').each(function (index, el) {
		$(el).on('mouseover', function (e) {
			...
			$(".nav_list_bg").stop(true, true).animate({
				"left": left
			}, 300);
		});
	});
