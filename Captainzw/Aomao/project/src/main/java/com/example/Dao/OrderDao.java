...
public interface OrderDao {
    ...
    @Select("SELECT orders.id,orders.money,orders_detail.num,goods.`name`,goods.price from orders INNER JOIN (orders_detail,goods) on orders.id=orders_detail.id and orders_detail.goodsid=goods.id WHERE orders.userid=#{userId} and orders.id=#{ordersId} ")
